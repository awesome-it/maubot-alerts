...
class TemplateRenderer:
    config: TemplateConfig | dict[str, Any] | None
    env: Environment

    def __init__(self, config: TemplateConfig | dict[str, Any] | None = None):
        self.config = config
        self.env = Environment(
            loader=FunctionLoader(self._load_template_source), autoescape=select_autoescape()
        )
